...
import arch.core.fupool.{ FuResp, FuReq, FunctionalUnitType }
...
class SchedulerDispatchIO(implicit p: Parameters) extends Bundle {
  val reqs = Vec(p(IssueWidth), Flipped(Decoupled(new FuReq)))
}

class SchedulerFuPoolIO(implicit p: Parameters) extends Bundle {
  val reqs = Vec(p(NumFUs), Decoupled(new FuReq))
...
final class SchedulerContext(val io: SchedulerIO)(implicit p: Parameters) {
  ...
  def isFuType(op: FuReq, t: FunctionalUnitType): Bool =
    op.fu_type === t.index.U(p(FuTypeWidth).W)

  def isLoad(op: FuReq): Bool =
    isFuType(op, FunctionalUnitType.FUNCTIONAL_UNIT_TYPE_LD)

  def isStore(op: FuReq): Bool =
  ...
  def defaultFuReqs(): Unit =
    for (i <- 0 until p(NumFUs)) {
      ...
      io.fu_pool.reqs(i).bits  := 0.U.asTypeOf(new FuReq)
    }
