...
RTTHREAD_STARTUP_OBJ ?= $(RTTHREAD_OBJ_DIR)/start-rtthread.o
...
rtthread-package: $(BAREMETAL_EXPORT_MK) $(RTTHREAD_STARTUP_OBJ) $(RTTHREAD_LIB) $(RTTHREAD_EXPORT_MK)

$(RTTHREAD_STARTUP_OBJ): $(STARTUP_SOURCE)
	@mkdir -p $(dir $@)
	$(RTTHREAD_CC) $(RTTHREAD_CFLAGS) $(RTTHREAD_ASFLAGS) -DSTART_ENTRY=rtthread_startup -c $< -o $@
...
$(RTTHREAD_EXPORT_MK): $(RTTHREAD_STARTUP_OBJ) $(RTTHREAD_LIB) $(BAREMETAL_EXPORT_MK)
	@mkdir -p $(dir $@)
	@printf '%s\n' \
		...
		'RTTHREAD_STARTUP_OBJ := $(RTTHREAD_STARTUP_OBJ)' \
		'RTTHREAD_LINKER_SCRIPT := $$(BAREMETAL_LINKER_SCRIPT)' \
